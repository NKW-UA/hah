...
o.length>0&&$("#img_shirt").attr("src","images/shirts/"+C[t].filename+"_"+o[0].filename+"_"+L+".png")}function c(){$("#centerLayoutContainer")
var t=s()
...
e+="bold"==t.getFontWeight()?'<label class="btn btn-default active"  id="texttoolbox_bold"><input type="checkbox" autocomplete="off" istool="bold" checked></label>':'<label class="btn btn-default"  id="texttoolbox_bold"><input type="checkbox" autocomplete="off" istool="bold"></label>',e+="italic"==t.getFontStyle()?'<label class="btn btn-default active" id="texttoolbox_italic"><input type="checkbox" autocomplete="off" istool="italic" checked></label>':'<label class="btn btn-default" id="texttoolbox_italic"><input type="checkbox" autocomplete="off" istool="italic"></label>',e+=t.getTextDecoration().indexOf("underline")>=0?'<label class="btn btn-default active" id="texttoolbox_underline"><input type="checkbox" autocomplete="off" istool="underline" checked></label>':'<label class="btn btn-default" id="texttoolbox_underline"><input type="checkbox" autocomplete="off" istool="underline"></label>',e+=t.getTextDecoration().indexOf("line-through")>=0?'<label class="btn btn-default active" id="texttoolbox_strikethrough"><input type="checkbox" autocomplete="off" istool="strikethrough" checked></label>':'<label class="btn btn-default" id="texttoolbox_strikethrough"><input type="checkbox" autocomplete="off" istool="strikethrough"></label>',e+=t.isEditing?'<label class="btn btn-default active" id="texttoolbox_edit""><input type="checkbox" autocomplete="off" istool="edit" checked></label>':'<label class="btn btn-default" id="texttoolbox_edit"><input type="checkbox" autocomplete="off" istool="edit"></label>',e+="</div>",e+='<div class="input-group colorpicker-component" id="texttoolbox_color">',e+='<span class="btn btn-default add-on"></span>',e+="</div>",$(".texttoolbox","#rightLayoutContainer").html(e),$("#texttoolbox_color").colorpicker({format:"hex",color:t.getFill()}).on("changeColor",b)}function b(t){var e=y.getActiveObject()
...
t&&t.isType("i-text")&&($("#texttoolbox_edit input").prop("checked")?t.enterEditing():t.exitEditing(),y.renderAll())}function _(t){$("#albumModal1").modal("hide"),fabric.Image.fromURL(t,function(t){y.add(t),t.getWidth()*d()>y.getWidth()/2&&t.scaleToWidth(y.getWidth()/2),t.viewportCenter().setCoords(),y.setActiveObject(t),y.renderAll()})}function _(t){$("#albumModal").modal("hide"),fabric.Image.fromURL(t,function(t){y.add(t),t.getWidth()*d()>y.getWidth()/2&&t.scaleToWidth(y.getWidth()/2),t.viewportCenter().setCoords(),y.setActiveObject(t),y.renderAll()})}var y,A,k,C={1:{filename:"men1",color:[{id:"1",filename:"blue",color:"0268b0"},{id:"2",filename:"white",color:"ffffff"}]},2:{filename:"women",color:[{id:"3",filename:"black",color:"000000"},{id:"4",filename:"white",color:"ffffff"}]}},w=570,T=20,L="front",j={},O=800,S=600,W=155,F=1e3,X=700
...
var G=new fabric.IText("",{fill:"#3C78FF",stroke:"#000",fontSize:F,fontFamily: "Roboto"})
...
return l(),!1}),$("#albumModal1").on("click",function(t){var e=$(t.target)
return e.is("img")&&e.attr("bgsrc")?(_(e.attr("bgsrc")),!1):void 0}),$("#albumModal").on("click",function(t){var e=$(t.target)
...
btnExport.addEventListener("click",function () {
    y.deactivateAll().renderAll();
});
			
		
var t=new fabric.IText("Hello",{fill:"#3C78FF",fontSize:F, fontFamily: "Roboto",})
$('#Poppins').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Poppins';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#Lato').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Lato';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#OpenSans').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Open Sans';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#Alegreya').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Alegreya';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#Pacifico').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Pacifico';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#BungeeShade').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Bungee Shade';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#Bungee').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Bungee';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#Monoton').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Monoton';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#GravitasOne').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Gravitas One';
    activeObject.setCoords();
                
    A.renderAll();
    A.calcOffset();
})
$('#Ewert').click(function changeFont() {
    var activeObject = A.getActiveObject();
    activeObject.fontFamily = 'Ewert';
    activeObject.setCoords();
